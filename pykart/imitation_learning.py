...
class ImitationLearner():
	...
	def __init__(self):
		...
		dist_logit = tf.contrib.layers.fully_connected(shared_fc, 1, activation_fn=None)
		dist_loss = tf.losses.mean_squared_error(predictions=dist_logit, labels=self.in_dist_center)
		pos_logit = tf.contrib.layers.fully_connected(shared_fc, 1, activation_fn=None)
		...
		self.state_loss = dist_loss
		self.total_loss = self.action_loss + self.state_loss + 1e-6*tf.losses.get_regularization_loss()
	...
	def train(self, training_data, validation_data):
		...
		util.save("supertuxkart-graph.tfg", session=self.sess)
